...
int main(int argc, string argv[])
{
    ...
    //to get the lenth of argv[1], the second command after ./substitution on command line
    int argv_length = strlen(argv[1]);
    //to make sure argv length must be 26 characters
    if (argv_length != 26)
    {
    ...
    for (int i = 0; i < plaintext_length; i++)      //iterate over the plaintext_Length
    {
        ...
        {
            printf("%c", toupper(argv[1][plaintext[i] -
                                         65]));  //calculation to print the encipher text amd make sure it is Uppercase (case doesn't change)
        }
        else if (islower(plaintext[i]))  //check if plaintext character is lowercase
        {
            printf("%c", tolower(argv[1][plaintext[i] -
                                         97]));  ///calculation to print the encipher text amd make sure it is lowercase(case doesn't change)
        }
